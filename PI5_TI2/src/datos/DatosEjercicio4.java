...
//		iniDatos("ficheros/Ejercicio4DatosEntrada3.txt");
	...
	public static Set<Integer> personasSet(){
		return personas.stream().map(p->p.id_persona()).collect(Collectors.toSet());
	}
	...
	public static Boolean distintaNacionalidad(Integer i ,Integer j) {
		Boolean res=false;
		if(getNacionalidad(i)!=getNacionalidad(j)) {
			res=true;
		}
		return res;
	}
	
	public static Boolean diferenciaEdad(Integer i ,Integer j) {
		Integer diferencia=Math.abs(getEdad(i)-getEdad(j));
		Boolean res= false;
		if(diferencia<=5) {
			res=true;
		}
	...
	public static List<Integer> getListaAfinidades(Integer i){
		return getAfinidades(i).values().stream().toList();
	}
