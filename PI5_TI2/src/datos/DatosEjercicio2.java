...
//		iniDatos("ficheros/Ejercicio2DatosEntrada3.txt");
	...
	public static Set<Integer> categorias;
	...
	public static Set<Integer> getCategorias(){
		return categorias;
	}
	...
	public static Integer getCategoria(Integer i,Integer j) {//cij
		Integer res=0;
		if(productos.get(i).categoria()==j) {
			res=1;
		}
		return res;
	}
	...
	public static List<Integer> presupuestoPorCategorias(){
		List<Integer> res= new ArrayList<Integer>();
		Integer numCat=getNumCategorias();
	...
	public static List<Integer> getListaPrecios(){
		return getProductos().stream().map(x->x.precio()).toList();
	}
	
	public static Integer idProdMasBaratoAPartirDe(Integer indice) {
		return getProductos().subList(indice, DatosEjercicio2.getNumProductos()).stream().min(Comparator.comparing(x->x.precio())).get().id_prod();
	}
